...
const SearchScreen = () => {
  ...
  const initMakers = () => {
    const initialMakers = operateurs.filter(item =>
      ...
      .map((item, index) => ({
        ...
        title: item.productions?.[0]?.nom || item.activites?.[0]?.nom || 'Activité',
      }));
  ...
  const handleMap = (data: any) => {
    ...
    if (data.event === 'onMapMarkerClicked') {
      const operateur = operateurs.find(op => 
        op.numeroBio && data.payload.mapMarkerId.includes(op.numeroBio.toString())
      );
      if (operateur) {
  ...
  const renderContent = () => {
    ...
    // Si pas de recherche, affiche la carte
    if (searchQuery.trim() === '') {
      return (
        <LeafletView
